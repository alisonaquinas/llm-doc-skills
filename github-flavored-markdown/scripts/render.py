#!/usr/bin/env python3
"""Render GitHub Flavored Markdown with cmark-gfm or Pandoc."""
...
def choose_renderer(target_format: str) -> str:
    ...
    if target_format == "html":
        raise FileNotFoundError(
            "Neither cmark-gfm nor pandoc was found. Install cmark-gfm for GitHub Flavored Markdown preview "
...
def build_command(renderer: str, input_path: str, target_format: str, output_path: str | None = None, gfm: bool = True, toc: bool = False) -> list[str]:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render GitHub Flavored Markdown.")
    ...
    parser.add_argument("--no-gfm", action="store_true", help="Disable explicit Pandoc gfm parsing")
...
def run(argv: list[str] | None = None) -> int:
    ...
    cmd = build_command(renderer=renderer, input_path=args.input, target_format=args.target_format, output_path=args.output, gfm=not args.no_gfm, toc=args.toc)
